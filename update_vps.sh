...
from sqlalchemy import Column, Integer, String, Text, Float, JSON, DateTime, text
...
async def smart_extract_products(url: str, max_products: int = 20, db: Session = Depends(get_db)):
    ...
    - Se for categoria: busca sitemap ou segue links para encontrar produtos
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            ...
            # Fazer request na URL fornecida
            response = await client.get(url, headers=headers)
            response.raise_for_status()
            ...
            if is_product_page:
                # É uma página de produto - extrair diretamente
                ...
                # É uma página de categoria/listagem - buscar produtos
...
def _is_product_page(soup, url):
    ...
    # Indicadores de página de produto
    product_indicators = [
        # URLs que indicam produto
        '/produto/' in url.lower(),
        '/product/' in url.lower(),
        '/p/' in url.lower(),
        '/item/' in url.lower(),
        
        # Elementos HTML que indicam produto
...
def _extract_product_data(soup, url):
    ...
    # Se não encontrou marca, extrair do domínio
